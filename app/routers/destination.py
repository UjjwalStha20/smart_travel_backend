...
class DestinationRouter:
    ...
    @router.get("/", response_model=list[DestinationRead] , status_code=200)
    def get_destinations(session: Session = Depends(get_session)) -> List[DestinationRead]:
        return DestinationService(session).get_all_destinations(offset=0, limit=100)
    

    @router.get("/{destination_id}")
    async def get_destination_by_id(session: SessionDep, destination_id: str) -> DestinationRead:
    ...
    @router.post("/")
    async def create_destination(session: SessionDep, destination: DestinationCreate) -> DestinationCreate:
    ...
    async def update_destination(session: SessionDep, destination_id: str, destination: DestinationUpdate) -> DestinationRead:
